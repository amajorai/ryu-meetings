...
mod tests {
    ...
    #[test]
    fn extract_json_object_handles_escapes_and_absence() {
        // An escaped quote inside the string must not end the string early.
        let s = r#"prefix {"a":"he said \"hi\" }"} suffix"#;
        let obj = extract_json_object(s).unwrap();
        assert_eq!(obj, r#"{"a":"he said \"hi\" }"}"#);
        // No object at all.
        assert!(extract_json_object("no braces here").is_none());
    }

    #[test]
    fn extract_json_object_nested_braces_balance() {
        let s = r#"{"outer":{"inner":1}}"#;
        assert_eq!(extract_json_object(s).unwrap(), s);
    }

    #[test]
    fn parse_notes_recovers_full_object_with_all_fields() {
        let notes = parse_notes(
            r#"{"summary":"s","key_points":["k"],"action_items":["a"],"decisions":["d"],"model":"ignored-here"}"#,
        );
        assert_eq!(notes.key_points, vec!["k"]);
        assert_eq!(notes.action_items, vec!["a"]);
        assert_eq!(notes.decisions, vec!["d"]);
    }

    #[tokio::test]
    async fn generate_notes_errors_when_gateway_unreachable() {
        // Non-empty transcript + an unused loopback port ⇒ "gateway unreachable".
