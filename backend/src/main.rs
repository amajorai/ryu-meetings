...
//!   `POST /api/host/capability/spaces.fileNotes` (ext-bearer + `x-ryu-plugin-id` authed, the
...
/// taken). Kept identical in `meetings.manifest.json`.
...
const MEETINGS_PLUGIN_ID: &str = "@ryu/meetings";
...
async fn main() -> anyhow::Result<()> {
    ...
    // endpoint is what makes the whole `/api/meetings` surface callable by an agent
    // (`ryu_meetings::api::openapi()` was dead code until now — only tests read it).
    //
    // Root, not under `/api/meetings`: Core tries the root FIRST and only falls back
    ...
    let meetings = Router::new()
        ...
        .route(
            "/openapi.json",
            get(|| async { Json(ryu_meetings::api::openapi()) }),
        )
...
impl MeetingsHost for SidecarMeetingsHost {
    ...
    async fn save_notes_to_space(&self, title: &str, markdown: &str) -> Option<(String, String)> {
        ...
        let resp = self
            .client
            .post(format!(
                "{}/api/host/capability/spaces.fileNotes",
                self.core_base
            ))
...
#[cfg(test)]
mod tests {
    use super::{bearer_ok, load_prefs, save_prefs, MeetingsHost, SidecarMeetingsHost};
    use std::collections::HashMap;
    ...
    fn ct_eq_is_length_and_content_sensitive() {
        ...
        assert!(!ryu_sidecar_runtime::constant_time_eq(b"abc", b"ab")); // length mismatch short-circuits
        assert!(!ryu_sidecar_runtime::constant_time_eq(b"ab", b"abc"));
    }

    #[tokio::test]
    async fn sidecar_host_pref_get_set_persist_to_disk() {
        let dir = std::env::temp_dir().join(format!(
            "ryu-meetings-host-test-{}-{}",
            std::process::id(),
            uuid::Uuid::new_v4().simple()
        ));
        let path = dir.join("meetings-prefs.json");
        let host = SidecarMeetingsHost::new(path.clone());

        assert!(host.pref_get("k").await.is_none());
        host.pref_set("k", "v").await.unwrap();
        assert_eq!(host.pref_get("k").await.as_deref(), Some("v"));

        // Durable: a second host over the same file sees the value.
        let reopened = SidecarMeetingsHost::new(path);
        assert_eq!(reopened.pref_get("k").await.as_deref(), Some("v"));
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[tokio::test]
    async fn generate_title_declines_short_summaries() {
        let dir = std::env::temp_dir().join(format!(
            "ryu-meetings-title-test-{}-{}",
            std::process::id(),
            uuid::Uuid::new_v4().simple()
        ));
        let host = SidecarMeetingsHost::new(dir.join("p.json"));
        // Under 16 chars ⇒ short-circuits to None without any Gateway call.
        assert!(host.generate_title("too short").await.is_none());
        let _ = std::fs::remove_dir_all(&dir);
    }
